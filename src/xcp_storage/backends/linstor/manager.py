...
class LinstorVolumeDetails:
    def __init__(self, number: int, max_allocated_size: int, capacity: int, default_storage_pool_name: str) -> None:
        ...
        # See resolution order here: https://linbit.com/drbd-user-guide/linstor-guide-1_0-en/#s-volumes_of_one_resource_to_different_storage_pools
        # - Volume definition
        # - Resource
        # - Resource definition
        # - Node
        # For this object, contains volume or resource definition storage pool name (if set).
        self.default_storage_pool_name = default_storage_pool_name

    def approximate_volume_size(self) -> int:
        volume_size = self.max_allocated_size
        if volume_size < 0:
            # Fallback in case of fetching issue.
            # Warning: the capacity may be smaller than the actual allocated size
            # that includes DRBD metadata; that is why this function includes the word "approximate".
            # This function should only be used for calculations related to simulated or not migrations for example.
...
class LinstorResourceReplicasState:
    def __init__(
        ...
        # Contains default storage pool list stored on resource definition property: `StorPoolName`.
        # OR the group storage pool list.
        # As a reminder, the `StorPoolName` property can also be read from a node to place resources,
        # however, since this data structure is global, it does not take that into account.
...
class LinstorManagerError(Exception):
    SHIFT_FETCH: Final   = 2
    SHIFT_CREATE: Final  = 5
    SHIFT_DESTROY: Final = 8
    SHIFT_OTHER: Final   = 11
    ...
    # Fetch (0x001C).
    ERR_NODE_FETCH: Final                = 1 << SHIFT_FETCH
    ERR_STORAGE_POOL_FETCH: Final        = 2 << SHIFT_FETCH
    ERR_RESOURCE_GROUP_FETCH: Final      = 3 << SHIFT_FETCH
    ERR_RESOURCE_DEFINITION_FETCH: Final = 4 << SHIFT_FETCH
    ERR_RESOURCE_FETCH: Final            = 5 << SHIFT_FETCH

    # Create (0x00E0).
    ERR_NODE_CREATE: Final                = 1 << SHIFT_CREATE
    ERR_NODE_INTERFACE_CREATE: Final      = 2 << SHIFT_CREATE
    ERR_STORAGE_POOL_CREATE: Final        = 3 << SHIFT_CREATE
    ERR_RESOURCE_GROUP_CREATE: Final      = 4 << SHIFT_CREATE
    ERR_RESOURCE_DEFINITION_CREATE: Final = 5 << SHIFT_CREATE
    ERR_RESOURCE_CREATE: Final            = 6 << SHIFT_CREATE

    # Destroy (0x0700).
    ERR_NODE_DESTROY: Final                = 1 << SHIFT_DESTROY
    ERR_NODE_INTERFACE_DESTROY: Final      = 2 << SHIFT_DESTROY
    ERR_STORAGE_POOL_DESTROY: Final        = 3 << SHIFT_DESTROY
    ERR_RESOURCE_GROUP_DESTROY: Final      = 4 << SHIFT_DESTROY
    ERR_RESOURCE_DEFINITION_DESTROY: Final = 5 << SHIFT_DESTROY
    ERR_RESOURCE_DESTROY: Final            = 6 << SHIFT_DESTROY

    # Other.
    ERR_NODE_NOT_EXISTS: Final                 = 1 << SHIFT_OTHER
    ERR_NODE_INTERFACE_MODIFY: Final           = 2 << SHIFT_OTHER
    ERR_NODE_INTERFACE_NOT_EXISTS: Final       = 3 << SHIFT_OTHER

    ERR_RESOURCE_GROUP_NOT_EXISTS: Final       = 4 << SHIFT_OTHER

    ERR_RESOURCE_DEFINITION_EXISTS: Final      = 5 << SHIFT_OTHER
    ERR_RESOURCE_DEFINITION_NOT_EXISTS: Final  = 6 << SHIFT_OTHER
    ERR_RESOURCE_DEFINITION_PROP_UPDATE: Final = 7 << SHIFT_OTHER

    ERR_RESOURCE_TOGGLE: Final                 = 8 << SHIFT_OTHER
    ERR_RESOURCE_PROP_UPDATE: Final            = 9 << SHIFT_OTHER
...
class LinstorManager(LinstorManagerBase):
    ...
    DEFAULT_PLACE_COUNT: Final = 2

    _ERR_MSG_NO_DATA: Final = "no data"
    _ERR_MSG_NODE_NOT_EXISTS: Final = "node doesn't exist"
    _ERR_MSG_NODE_INTERFACE_NOT_EXISTS: Final = "node interface doesn't exist"
    _ERR_MSG_RESOURCE_DEFINITION_EXISTS: Final = "definition exists"
    _ERR_MSG_RESOURCE_DEFINITION_NOT_EXISTS: Final = "definition doesn't exist"
    _ERR_MSG_RESOURCE_GROUP_NOT_EXISTS: Final = "group doesn't exist"

    _PROP_AUTO_PLACE_TARGET: Final = "AutoplaceTarget"
    _PROP_PREF_NIC: Final = "PrefNic"
    _PROP_STORAGE_POOL_NAME: Final = "StorPoolName"

    _DEFAULT_NIC: Final = "default"
    ...
    def get_controller_resource_names(
        ...
        *,
        ...
        resource_groups = self._fetch_resource_groups(group_names) if group_names else {}
    ...
    def get_controller_resource_replicas(
        self,
        group_names: Optional[Set[str]] = None,
        ...
        context = LinstorManager._ResourceReplicasGetterContext(
            resource_groups,
            excluded_node_names,
            mode,
            self._fetch_nodes(),
            self._fetch_resource_entries()
        )

        for resource_definition in self._fetch_resource_definitions().values():
            self._get_resource_replicas(resource_definition, context)

        return context.resource_replicas
    ...
    def remove_controller_skip_disks(self) -> None:
        ...
        for resource_entry in resource_entries.values():
            ...
            if not any(
                self._is_active_resource(resource) and self._is_up_to_date_resource(resource)
                for resource in resources
            ...
            for resource in skip_disk_resources.values():
                try:
                    if not self._is_diskless_resource(resource):
    ...
    def get_node_preferred_interface_name(self, node_name: str) -> str:
        ...
        return node.properties.get(self._PROP_PREF_NIC, self._DEFAULT_NIC)
    ...
    def set_node_preferred_interface_name(self, node_name: str, interface_name: str) -> None:
        ...
        errors = self._filter_errors(self._exec_query(
            ...
            node_name=node_name,
            property_dict={self._PROP_PREF_NIC: interface_name}
        ))
        if not errors:
            return
    ...
    def destroy_node_interface(self, interface_name: str, node_name: str) -> None:
        if interface_name == self._DEFAULT_NIC:
            raise LinstorManagerError(
        ...
        for resource_definition in self._fetch_resource_definitions().values():
            ...
            for volume_definition in resource_definition.volume_definitions:
                ...
                if size > 0:
                    ...
                    logger.warning(
                        "Invalid volume definition size detected on `%s/%d: %d.",
    ...
    def get_resource_definition_default_place_count(self, resource_name: str) -> int:
        ...
        if resource_definition:
            resource_group = self._fetch_one_resource_group(resource_definition.resource_group_name)
            # Note: If we have the definition but not the resource group,
            # there is a bug or race condition with another call.
            if resource_group:
                return self._get_resource_group_place_count(resource_group)
    ...
    def place_resource(self, resource_name: str, place_count: Optional[int] = None) -> None:
        ...
        errors = self._filter_errors(self._exec_query(
            ...
            rsc_name=resource_name,
            storage_pool=None, # Automatically select a storage pool in the resource group or keys like `StorPoolName`.
            place_count=place_count,
            diskless_on_remaining=False
        ))
        if not errors:
    ...
    def get_resource_in_use_node_names(self, resource_name: str) -> List[str]:
        # Note: we don't return a simple string here because a DRBD can have two primaries
        # using dual-primary mode config.
        resource_entry = self._fetch_one_resource_entry(resource_name)
        if not resource_entry:
            return []
        return [resource_state.node_name for resource_state in resource_entry.states if resource_state.in_use]
    ...
    def get_resource_replicas(
        ...
        context = LinstorManager._ResourceReplicasGetterContext(
            ...
            mode,
            self._fetch_nodes(),
            self._fetch_resource_entries({resource_name})
        )
    ...
    def _fetch_resource_entries(self, name_filter: Optional[Set[str]] = None) -> Dict[str, _ResourceEntry]:
    ...
    @functools.lru_cache(maxsize=128)
    def _fetch_one_resource_entry(self, resource_name: str) -> Optional[_ResourceEntry]:
        return next(iter(self._fetch_resource_entries_uncached((resource_name, )).values()), None)

    @functools.lru_cache(maxsize=1)
    def _fetch_resource_entries_impl(self, name_filter: Tuple[str, ...]) -> Dict[str, _ResourceEntry]:
        return self._fetch_filter(self._fetch_resource_entries_uncached(()), name_filter)

    def _fetch_resource_entries_uncached(self, name_filter: Tuple[str, ...]) -> Dict[str, _ResourceEntry]:
        def raise_error(reason: str) -> Never:
            raise LinstorManagerError(
        ...
        resource_entries: Dict[str, LinstorManager._ResourceEntry] = defaultdict(LinstorManager._ResourceEntry)
    ...
    # ----------------------------------
    # Controller helpers impl.
    # ----------------------------------

    class _ResourceReplicasGetterContext:
        def __init__(
            self,
            # User params.
            resource_groups: Dict[str, linstor.responses.ResourceGroup],
            excluded_node_names: Optional[Set[str]],
            mode: LinstorResourceReplicasMode,
            # Cache.
            nodes: Dict[str, linstor.responses.Node],
            resource_entries: Dict[str, "LinstorManager._ResourceEntry"]
        ) -> None:
            self.resource_groups = resource_groups
            self.excluded_node_names = excluded_node_names
            self.mode = mode
            self.nodes = nodes
            self.resource_entries = resource_entries
            # Result.
            self.resource_replicas: List[LinstorResourceReplicas] = []

    def _get_resource_replicas( # noqa: C901
        self,
        resource_definition: linstor.responses.ResourceDefinition,
        context: _ResourceReplicasGetterContext
    ) -> None:
        # 0. Processing one resource...
        group_name = resource_definition.resource_group_name
        resource_group = context.resource_groups.get(group_name)
        if not resource_group:
            return

        resource_entry = context.resource_entries.get(resource_definition.name)
        if not resource_entry:
            return
        ...
        default_storage_pool_name = self._get_default_storage_pool_name(resource_definition)
        ...
        for volume_definition in resource_definition.volume_definitions:
            ...
            volume_details.default_storage_pool_name = \
                self._get_default_storage_pool_name(volume_definition) or default_storage_pool_name

        # 2. Process node names and volume allocated sizes.
        diskful_node_names = []
        diskless_node_names = []
        ...
        for resource in resource_entry.resources:
            ...
            node = context.nodes.get(resource.node_name)
            if not node:
        ...
        # 3. Compute counts.
        if context.excluded_node_names:
            def count_helper(node_names: List[str]) -> int:
                assert context.excluded_node_names # To make `mypy` happy.
                return sum(node_name not in context.excluded_node_names for node_name in node_names)
        ...
        if (
            # All.
            context.mode == LinstorResourceReplicasMode.ALL or
            # Simple checks.
            (diskful_count < expected_place_count or diskful_count + diskless_count < 3) or
            # Strict.
            (
                context.mode == LinstorResourceReplicasMode.STRICT_DATA_INTEGRITY and
            ...
            context.resource_replicas.append(LinstorResourceReplicas(
                resource_definition.name,
    ...
    # ----------------------------------
    # `is`/`as` and generic helpers.
    # ----------------------------------

    @classmethod
    def _get_default_storage_pool_name(cls, obj: Union[
        linstor.responses.Node,
        linstor.responses.ResourceDefinition,
        linstor.responses.VolumeDefinition
    ]) -> str:
        return obj.properties.get(cls._PROP_STORAGE_POOL_NAME, "")

    @staticmethod
    def _is_online_node(node: linstor.responses.Node) -> bool:
        return node.connection_status == "ONLINE"
    ...
    def _is_up_to_date_resource(cls, resource: linstor.responses.Resource) -> bool:
        return all(map(cls._is_up_to_date_volume, resource.volumes))
